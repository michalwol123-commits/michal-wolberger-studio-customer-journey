...
Deno.serve(async (req) => {
  try {
    ...
    if (!file_url || !project_id) {
      return Response.json({ error: 'Missing file_url or project_id' }, { status: 400 });
    }

    // Use LLM to extract design items from the PDF
    const result = await base44.asServiceRole.integrations.Core.InvokeLLM({
      prompt: `You are an interior design project assistant. Analyze this design consultation summary document (PDF).
...
Keep all Hebrew text as-is. Extract ALL URLs mentioned.`,
    ...
    // Return extracted items for user review (don't save yet)
